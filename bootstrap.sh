...
DECRYPTED_SWAP_UUID=$(ls -lah /dev/disk/by-uuid/ | \
                          grep $DM_SWAP | awk '{print $9}')
# enable swap on the decrypted cryptswap
swapon /dev/disk/by-uuid/$DECRYPTED_SWAP_UUID

DECRYPTED_ROOT_UUID=$(ls -lah /dev/disk/by-uuid/ | \
                          grep $DM_ROOT | awk '{print $9}')
# mount the decrypted cryptroot to /mnt (btrfs)
mount -o rw,noatime,compress=zstd,ssd,space_cache \
      /dev/disk/by-uuid/$DECRYPTED_ROOT_UUID /mnt
...
mount -o rw,noatime,compress=zstd,ssd,space_cache,subvol=@home \
      /dev/disk/by-uuid/$DECRYPTED_ROOT_UUID /mnt/home
# mount @var subvolume to /mnt/var
mount -o rw,noatime,compress=zstd,ssd,space_cache,subvol=@var \
      /dev/disk/by-uuid/$DECRYPTED_ROOT_UUID /mnt/var
...
cat /mnt/etc/nixos/hardware-configuration.nix | grep -v "boot\.initrd\.luks" | \
    grep -v "^}\$" > /mnt/etc/nixos/hardware-configuration.nix_tmp

CRYPTKEY_UUID=$(ls -lah /dev/disk/by-uuid/ | \
                    grep $(basename $DISK_CRYPTKEY) | awk '{print $9}')
ROOT_UUID=$(ls -lah /dev/disk/by-uuid/ | \
                grep $(basename $DISK_ROOT) | awk '{print $9}')
SWAP_UUID=$(ls -lah /dev/disk/by-uuid/ | \
                grep $(basename $DISK_SWAP) | awk '{print $9}')
...
diff /mnt/etc/nixos/hardware-configuration.nix \
     /mnt/etc/nixos/hardware-configuration.nix_tmp

rm /mnt/etc/nixos/hardware-configuration.nix

mv /mnt/etc/nixos/hardware-configuration.nix_tmp \
